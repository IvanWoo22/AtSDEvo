...
- 独立 RepeatMasker soft-mask 用于完整流程敏感性分析；
- 源小写区域不能等同于当前 GFF 重复注释的完整集合。

GFF 扩展掩膜 51,953,668 bp（36.464%），独立 RM 掩膜 37,623,317 bp
（26.406%），两者 Jaccard 为 0.537。RM 不是 GFF 扩展的简单放宽版本。
...
RM 全流程得到 1,892 个严格二拷贝事件、864 个稳定 P/D 事件、262 个 P0 队列
事件和 237 个 SNP 主终点事件。SNP P=11,723、D=19,912、D/P=1.699。
RO≥0.80 的 144 对共同 SNP 事件中，GFF 与 RM 的 D/P 分别为 1.679 和 1.701。
Micro-indel 在两种掩膜下均为 D>P，但精确效应量变化更大。
...
- BISER 边集和事件边界受掩膜定义影响，全集样本量不可直接作生物学计数比较。
...
7. 对 RO≥0.80 跨掩膜配对事件做长度、重复比例与功能背景的配对模型；
8. 评估预先定义的 RM∪GFF-selected-repeat 混合掩膜，但不并入当前主结果。
